...
%Nós apresentamos dois fluxos de trabalho para learnersourcing dicas que atribuem tarefas de geração de dica para os alunos com base no que problemas o aluno tem resolvido recentemente. No fluxo de trabalho auto-reflexão, os alunos gerar sugestões, refletindo sobre um obstáculo que eles próprios tenham superado recentemente. 

%No fluxo de trabalho de comparação, os alunos comparar as suas próprias soluções para os de outros estudantes, gerando uma dica como um subproduto de explicar como se pode obter a partir de uma solução para o outro. 

%O segundo fluxo de trabalho pode operar sobre a saída do primeiro, conforme mostrado na Figura 1. Em ambos os fluxos de trabalho, o aspecto chave é que, através da sua própria experiência lutando com um problema particular, os alunos podem se tornar especialistas sobre as otimizações específicas que implementam e bugs eles resolvem. Os fluxos de trabalho podem tirar a pressão de docentes, dando aos alunos as experiências educacionais valiosos de reflexão e de geração de explicações.
...
Nesta subseção será explicado as duas etapas que serão realizadas para responder as duas questões de pesquisa. A primeira etapa consiste em aplicar um estudo controlado na Universidade Tecnológica Federal do Paraná para gerar os dados de \foreign{log} de execução das listas de exercícios necessários. Desto modo, após a geração dos dados de \foreign{log} a segunda etapa será inicializada. A segunda etapa apresenta as analises que serão feitas para responder as duas questões de pesquisa.
